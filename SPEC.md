# BiteLedger — Build Specification

> Batch 21AUG, app 02 of 30. This document is the complete brief for
...
**One-line positioning:** Your intake, double-entry.

| Field | Value |
| --- | --- |
| Product name | BiteLedger |
| Bundle identifier | `com.biteledger.ledger` |
| Domain | https://biteledger.pro |
| Contact URL | https://biteledger.pro/contact-us |
| Deployment target | iOS 17.0 |
| Swift version | 6.2, strict concurrency `complete` |
| Devices | iPhone only, portrait |
| Interface style | Light |
| Asset prefix | `blg_` |
| User-Agent | `BiteLedger/1.0 (iOS; +https://biteledger.pro)` |
...
7. **The app must build clean** with
   `xcodegen generate && xcodebuild -scheme BiteLedger -destination 'generic/platform=iOS' build`.
...
## 3. Uniqueness assignment for BiteLedger
...
| UI approach | **UIKit + Storyboards + segues** |
| Naming convention | **Prefix-based (`BLG`)** |
| File organization | **By feature** |
| Dependency strategy | **SPM remote x1 — swift-collections** |
| Design direction | **Ledger / accounting paper** |
| Typography | **Georgia** |
| Navigation pattern | **Side menu (hamburger drawer)** |
| AI art style | **Vintage engraving / banknote etching** |
| Functional twist | **Double-entry ledger + CSV export** |
| Persistence | **SQLite via raw C API (libsqlite3)** |
| Screen composition | see 3.6 |
| Scanner technology | **AVCaptureMetadataOutput, live, restricted rectOfInterest** |
| Search endpoint | **/api/v2/search with an explicit fields list, page_size 16** |
| Day key representation | **String in yyyy-MM-dd form** |

### 3.1 Architecture contract

Classic MVVM. Each screen has a `ViewModel` exposing `@Published` outputs and
accepting `PassthroughSubject` inputs. All async work is Combine publishers
(`URLSession.dataTaskPublisher`, `.map`, `.decode`, `.receive(on:)`). View
controllers bind via `.sink` into a `Set<AnyCancellable>`. No async/await in the
presentation layer; repositories may bridge to Combine with `Future`.
...
One `Main.storyboard` holding every scene, wired with segues. Cells are
prototype cells defined inside the storyboard. `@IBOutlet`/`@IBAction` used
throughout. Set `INFOPLIST_KEY_UIMainStoryboardFile: Main` and pass
`prepare(for:sender:)` payloads between scenes.

### 3.3 Naming contract

Convention: Prefix-based (`BLG`).

Examples to follow: `BLGLedgerViewController`, `BLGEntryCell`, `BLGAccountService`, `BLGDayViewModel`

Apply it to types, files, properties, methods and asset names alike.

### 3.4 Dependency contract

`apple/swift-collections` for `OrderedDictionary` backing the ledger index.

### 3.5 Navigation contract

Custom container VC sliding a left drawer over the ledger. No tab bar. Drawer lists all sections.
...
Drawer with seven destinations. Detail and Assign are separate storyboard
scenes reached by segue. The log is a full 'Statement' scene with a running
balance column. Today is the ledger front page.
...
Technology: **AVCaptureMetadataOutput, live, restricted rectOfInterest**

Narrow horizontal scan window drawn as a ruled ledger line.
...
- Open Food Facts search uses **/api/v2/search with an explicit fields list, page_size 16**.
- A day is represented internally as **String in yyyy-MM-dd form**. Use this consistently in
  storage, in queries and in identifiers.
- The demo seed flag key is `blg.demo.v1`.
- `NSCameraUsageDescription` is exactly: "BiteLedger reads barcodes to post entries into your food ledger."
...
```
BiteLedger/
  Ledger/ Lookup/ Scan/ Planner/ Wishlist/ Goals/ Shared/
  Assets.xcassets/
```
...
### 5.6 Assign
Choose a slot — Opening Entry, Midday Entry, Closing Entry, Petty Cash — and choose
eaten-today or a future date. Petty Cash is eaten-only and must be unavailable
...
https://biteledger.pro/contact-us.
...
Direction: **Ledger / accounting paper**

### 7.1 Palette

| Token | Hex | Use |
| --- | --- | --- |
| `background` | `#F5F0E1` | Screen background |
| `surface` | `#FFFDF5` | Cards, rows, sheets |
| `ink` | `#3E2F1C` | Primary text and icons |
| `accent` | `#C0392B` | Primary action, key figure, progress fill |
| `muted` | `#A08B6F` | Secondary text, dividers, disabled |
...
Georgia for all text; tabular figures via `UIFont.monospacedDigitSystemFont` only for number columns.
...
Chosen technology: **SQLite via raw C API (libsqlite3)**

Hand-written SQLite wrapper over `sqlite3_*`. Tables: entries, targets, wishlist, plans. WAL mode.
...
Set `User-Agent: BiteLedger/1.0 (iOS; +https://biteledger.pro)` on every request. Never reuse another app's string.
Use the **/api/v2/search with an explicit fields list, page_size 16** search endpoint for this app.
...
INFOPLIST_KEY_NSCameraUsageDescription: BiteLedger reads barcodes to post entries into your food ledger.
...
## 12. Functional twist: Double-entry ledger + CSV export

Every entry is shown as a debit against the day's calorie budget with a running
balance column. A 'Statement' screen renders the month and exports CSV via
...
Art style: **Vintage engraving / banknote etching**
...
given. Every name carries the `blg_` prefix. No asset may be reused
...
```bash
sips -g pixelWidth -g pixelHeight -g hasAlpha blg_AppIcon.png
...
```bash
sips -s format jpeg blg_AppIcon.png --out tmp.jpg \
  && sips -s format png tmp.jpg --out blg_AppIcon.png && rm tmp.jpg
```
...
| --- | --- | --- | --- | --- |
| 1 | `blg_AppIcon` | 1024x1024 | **NO** | App Store icon. NO alpha channel, NO transparency, NO text, NO rounded corners, NO drop shadow outside the canvas. |
| 2 | `blg_Splash` | 1290x2796 | allowed | Launch background. The middle third must stay quiet so the wordmark reads on top. |
| 3 | `blg_Onboarding1` | 1024x1536 | allowed | Onboarding page 1 illustration: what the app is for. |
| 4 | `blg_Onboarding2` | 1024x1536 | allowed | Onboarding page 2 illustration: scan and search. |
| 5 | `blg_Onboarding3` | 1024x1536 | allowed | Onboarding page 3 illustration: setting daily targets. |
| 6 | `blg_EmptyLog` | 1024x1024 | allowed | Empty state: nothing logged today yet. Calm and inviting, never sad. |
| 7 | `blg_EmptySearch` | 1024x1024 | allowed | Empty state: search returned no products. |
| 8 | `blg_EmptyPlan` | 1024x1024 | allowed | Empty state: no planned entries. |
| 9 | `blg_EmptyWish` | 1024x1024 | allowed | Empty state: wish list has no items. |
| 10 | `blg_SlotOpeningEntry` | 512x512 | allowed | Icon for the 'Opening Entry' slot. |
| 11 | `blg_SlotMiddayEntry` | 512x512 | allowed | Icon for the 'Midday Entry' slot. |
| 12 | `blg_SlotClosingEntry` | 512x512 | allowed | Icon for the 'Closing Entry' slot. |
| 13 | `blg_SlotPettyCash` | 512x512 | allowed | Icon for the 'Petty Cash' slot. |
| 14 | `blg_MacroProtein` | 512x512 | allowed | Macro icon: protein. Must be visually distinct from carbs and fat at 24pt. |
| 15 | `blg_MacroCarbs` | 512x512 | allowed | Macro icon: carbohydrates. |
| 16 | `blg_MacroFat` | 512x512 | allowed | Macro icon: fat. |
| 17 | `blg_ProductPlaceholder` | 600x600 | allowed | Fallback thumbnail shown when a product has no image. Used in every list row. |
| 18 | `blg_CardBackdrop` | 1200x800 | allowed | Backdrop art for the product detail card. Low contrast so text stays readable. |
| 19 | `blg_Texture` | 2048x2048 | allowed | Tiling background texture used at low opacity behind content. MUST tile with no visible seam. |
| 20 | `blg_ControlFace` | 512x512 | allowed | Custom control artwork used for the primary interactive element. |
| 21 | `blg_ScanOverlay` | 1024x1024 | required | Camera overlay art framing the barcode. The centre must be fully transparent. |
| 22 | `blg_TwistHero` | 1024x1024 | allowed | Hero art for the 'Double-entry ledger + CSV export' feature screen. |
| 23 | `blg_SuccessMark` | 512x512 | allowed | Shown briefly when a food is logged successfully. |
| 24 | `blg_HeaderDecor` | 1200x600 | allowed | Decorative header accent on the main screen. |
...
- `blg_Texture` must tile seamlessly. Verify by placing four copies
  edge to edge.
- `blg_ScanOverlay` needs a genuinely transparent centre.
...
| Pasta Spaghetti | `8076800195057` | 371 | 13.0 | 74.7 | 1.5 |
...
| Role | Label in BiteLedger | Planned | Eaten |
| --- | --- | --- | --- |
| Morning meal | Opening Entry | yes | yes |
| Midday meal | Midday Entry | yes | yes |
| Evening meal | Closing Entry | yes | yes |
| Snack | Petty Cash | no | yes |
...
Add a unit test target `BiteLedgerTests` covering at minimum:
...
- [ ] `xcodebuild -scheme BiteLedger -destination 'generic/platform=iOS' build` succeeds.
...
- [ ] All four slots accept entries; Petty Cash rejects future dates.
...
**Uniqueness**
- [ ] Architecture matches **MVVM + Combine** with no leakage across layers.
- [ ] UI approach matches **UIKit + Storyboards + segues**.
- [ ] Navigation matches **Side menu (hamburger drawer)**.
- [ ] Screen composition follows section 3.6, not the generic list in section 5.
- [ ] Scanner uses **AVCaptureMetadataOutput, live, restricted rectOfInterest**.
- [ ] Day key is **String in yyyy-MM-dd form**.
...
- [ ] Demo seed runs only on Simulator, once, behind `blg.demo.v1`.
...
**Assets**
- [ ] All 24 images generated in the **Vintage engraving / banknote etching** style, prefixed `blg_`.
